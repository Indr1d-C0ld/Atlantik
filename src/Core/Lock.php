 ...
 *
 * Servono a una cosa sola: impedire che due processi facciano avanzare lo
 * stesso battello nello stesso momento. Il battito del minuto e la richiesta
 * del giocatore sono processi diversi — il flock di bin/tick.php protegge solo
 * i battiti fra loro — e senza lucchetto possono partire dallo stesso
 * last_sim_gts, simulare la stessa finestra e applicarne due volte gli effetti:
 * righe di giornale doppie, contatti aperti due volte, prove di avaria tirate
 * due volte.
 *
...
final class Lock
{
    ...
    private static function nomeCompleto(string $nome): string
    {
        ...
        return substr('atl:' . (string) Config::get('db.name', 'atlantik') . ':' . $nome, 0, 64);
    }
