...
final class GameConfig
{
    ...
    private static function load(): array
    {
        ...
        $rows = [];
        try {
            // La nota viene letta insieme al resto: serve a chi guarda il
            // pannello per sapere che cosa muove quella chiave. Prima non si
            // leggeva, e la colonna delle note era vuota per tutte e
            // quarantuno — le descrizioni c'erano, in tabella, e non le
            // vedeva nessuno.
            foreach (Database::all('SELECT ckey, cvalue, ctype, note FROM game_config') as $r) {
                $rows[(string) $r['ckey']] = [
